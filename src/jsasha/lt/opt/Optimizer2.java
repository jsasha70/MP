 ...
 * 
 */
public class Optimizer2 extends LT_dataSrc {
  ...
  public LogicTree run(LogicTree lt) {
    ...
    for (int i = one + 1; i < len1; i++) {
      if (interrupted) {
        node = add(tr[lt.getRel1(i)], tr[lt.getRel2(i)]);
        tr[i] = node;
      } else {
        len_orig = len;
        node = add(tr[lt.getRel1(i)], tr[lt.getRel2(i)]);
